...
let rec at idx lst =
  ...
  | 0, _ ->
      Some (List.hd lst)
  | _ ->
      at (idx - 1) (List.tl lst)

(* p4 *)
let length lst =
  let rec length2 n = function [] -> n | _ :: tl -> length2 (n + 1) tl in
  length2 0 lst

(* p5 *)
let rev lst =
  let rec rev2 out = function [] -> out | hd :: tl -> rev2 (hd :: out) tl in
  rev2 [] lst
...
let flatten lst =
  let rec flatten_node acc = function
    | One x ->
        x :: acc
    | Many l ->
        List.fold_left flatten_node acc l
  in
  List.fold_left flatten_node [] lst |> List.rev
...
let rec compress = function
  ...
  | [x] ->
      [x]
  | x :: xs ->
      if x = List.hd xs then compress xs else x :: compress xs

(* p9 *)
let rec pack_aux el rl = function
  | [] ->
      rl
  | [x] ->
      (x :: el) :: rl
  | hd :: tl ->
      if hd = List.hd tl then
        pack_aux (hd :: el) rl tl
      else
        pack_aux [] ((hd :: el) :: rl) tl

let pack lst = pack_aux [] [] lst |> List.rev

(* p10 *)
(* encode [1;1;2;2;2;3;3;4;4;3] = [(2, 1); (3, 2); (2, 3); (2, 4); (1, 3)] *)
let rec encode_aux n acc = function
  | [] ->
      acc
  | [x] ->
      (n + 1, x) :: acc
  | hd :: tl ->
      if hd = List.hd tl then
        encode_aux (n + 1) acc tl
      else
        encode_aux 0 ((n + 1, hd) :: acc) tl

let encode lst = encode_aux 0 [] lst |> List.rev

(* p11 *)
type 'a rle = One of 'a | Many of int * 'a
(* encode' [1;1;2;2;2;3;3;4;4;3] = [Many (2, 1); Many (3, 2); Many (2, 3); Many (2, 4); One 3] *)

let encode' lst =
  let encoded = encode lst in
  let rle_of_tuple (n, x) = match n with 1 -> One x | _ -> Many (n, x) in
  List.map rle_of_tuple encoded

(* p12 *)
(* decode [Many (2, 1); Many (3, 2); Many (2, 3); Many (2, 4); One 3] = [1; 1; 2; 2; 2; 3; 3; 4; 4; 3] *)
let list_of_rle = function
  | One x ->
      [x]
  | Many (n, x) ->
      List.init n (fun _ -> x)
...
let rec decode_aux acc = function
  | [] ->
      acc
  | hd :: tl ->
      decode_aux (acc @ list_of_rle hd) tl

let decode lst = decode_aux [] lst

(* p13 *)
let rle_new x = function 1 -> One x | n -> Many (n, x)

let rec encode_d_aux n acc = function
  | [] ->
      acc
  | [x] ->
      rle_new x (n + 1) :: acc
  | hd :: tl ->
      if hd = List.hd tl then
        encode_d_aux (n + 1) acc tl
      else
        encode_d_aux 0 (rle_new hd (n + 1) :: acc) tl

let encode_d lst = encode_d_aux 0 [] lst |> List.rev

(* p14 *)
(* duplicate ["a";"b";"c";"c";"d"];;
    - : string list = ["a"; "a"; "b"; "b"; "c"; "c"; "c"; "c"; "d"; "d"] *)
let duplicate lst = List.map (fun a -> [a; a]) lst |> List.flatten
...
let replicate lst n =
  let repli_ele n a = List.init n (fun _ -> a) in
  List.map (repli_ele n) lst |> List.flatten
...
let drop lst n =
  let rec drop_aux i = function
    | [] ->
        []
    | h :: t ->
        if i = n then drop_aux 1 t else h :: drop_aux (i + 1) t
  in
  drop_aux 1 lst
...
let rotate lst n =
  let i = if n < 0 then List.length lst + n else n in
  split lst i |> List.rev |> List.concat
...
(* ##### 20. Remove the K'th element from a list. (easy)
   ...
   The first element of the list is numbered 0, the second 1,...
       ...
       # remove_at 1 ["a";"b";"c";"d"];;
       - : string list = ["a"; "c"; "d"] *)
let remove_at n lst = List.filteri (fun i _ -> i <> n) lst
...
let range d1 d2 =
  let n = abs (d1 - d2) + 1 in
  let g i = if d1 < d2 then min d1 d2 + i else max d1 d2 - i in
  List.init n g
...
let rand_select lst n =
  ...
  let rec aux acc n len lst =
    ...
    | n, lst ->
        let i = rand_i len in
        let ele = at i lst |> Option.get in
        let acc' = ele :: acc in
        let lst' = remove_at i lst in
        aux acc' (n - 1) (len - 1) lst'
  in
  aux [] n len lst
...
(*
   ...
   *Be careful* that in OCaml, the definition of Leaf is different from typical imperative programming like Java. `Empty` is like `Null` and definition of `Leaf` here is the `Node` whose both left and right children are `Empty`.*
   ...
   A [complete binary tree](https://www.scaler.com/topics/complete-binary-tree/) with height H is defined as follows: The levels 1,2,3,...,H-1 contain the maximum number of nodes (i.e 2i-1 at the level i, note that we start counting the levels from 1 at the root). In level H, which may contain less than the maximum possible number of nodes, all the nodes are "left-adjusted". This means that in a levelorder tree traversal all internal nodes come first, the leaves come second, and empty successors (the nil's which are not really nodes!) come last.
